...
import { splitLyrics } from '@/assets/js/lrcUtil.js';
	...
	methods: {
		...
		handleLrc(hash, id, accesskey) {
			...
			reqLyricsInfo(id, accesskey, function(result) {
				...
				// var lrcInfo = krcParser(testBase64);

				//获取base64歌词字符串
				var lrcInfo = krcParser(result.content);
				that.splitLrc(lrcInfo);
			});
		},
		/**
		 * 分割歌词
		 * @param {Object} lrcInfo
		 */
		splitLrc(lrcInfo) {
			splitLyrics(lrcInfo, ctx, 50, this.viewWidth);
		}
	}
};
</script>
